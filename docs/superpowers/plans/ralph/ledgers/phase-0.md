...
- [x] U3: Ensure a runtime parity Playwright test exists (`Object.keys(window)` ⊇ golden + zero console errors on boot) against the served page; add it if missing.
...
- U3 (iter 1): added `tests/playwright/test_parity_runtime.py` — boots the SPA (license pre-seeded, single clean boot), asserts `Object.keys(window)` ⊇ golden(functions∪objects) and zero console/page errors (conftest noise filter). Green against :8001. Tightens automatically post-split (bridge floor).
- GATE (iter 1): `pytest tests/parity -q` = 3 passed, 1 skipped (post-split window-bridge test correctly inert pre-`js/`); runtime parity = 1 passed on :8001; `api/static/index.html` UNCHANGED (git diff + status both empty). All 3 units [x]. GATE PASSED: phase-0.
