...
- [~] U7 workspace-legacy/ — COMPONENTS relocated: ComposerWorkstream/AgentTuning/ChatRating/ComposerSplit/BootSequence → workspace-legacy/*.js (window-bridged; ComposerSplit stays in export block; their df* refs resolve via the existing window-bridge at runtime — runtime-parity clean-boot confirms no eval-time df* use). ScaffoldModal/Pins/Threads aren't top-level IIFEs. **df* → canvas.js DEFERRED to phase-3**: the 41 df* functions mutate the reassigned dfEditor/dfNodeData/dfNextId state (the U2-deferred lexicals); moving them needs the state-object accessor rewrite, which IS phase-3's boot/ordering work. Bridging df* is already done (U1). Fast VERIFY 165.
