...
| 6.2 Arch scheduler consumes MCP overhead | ✅ this PR | Module-level `mcp_overhead_gb()` helper in `api/services/deployment.py` reads the live pool overhead (returns 0.0 if the pool isn't importable or accounting fails — never blocks a memory query); every `Deployment.effective_memory_gb()` impl (DMG / container / host_native) subtracts it after headroom, clamped at 0; the `/api/system/architecture` deployment payload exposes the new `mcp_overhead_gb` field so operators can see why the budget dropped |
| 4, 7.2 + 7.3 | ⬜ not started | see reconciliation above |
