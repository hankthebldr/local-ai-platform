...
| 4.2 + 4.3 (engine-side capture in step_executor) | ⬜ deferred | Needs the engine to actually invoke MCPs from steps via the runner pool (the wiring deferred from Phase 2). Schema is now ready; the wiring PR can just write to the fields |
...
| 4.2 + 4.3 (engine-side capture in step_executor) | ⬜ remaining | The last piece. Schema (4.1) + run rollup (4.4) + engine lifecycle (Phase 2 engine wiring) are all in. Wiring step_executor to capture per-step skill activations + MCP/plugin tool calls writes to fields that already exist on `StepResult` |
