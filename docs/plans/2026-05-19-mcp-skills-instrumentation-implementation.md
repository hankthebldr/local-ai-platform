...
| 2 engine-side pool lifecycle | ✅ this PR | `WorkflowEngine._safe_drain_mcp_pool(run)` calls `pool.release_workflow(run_id)`, dedupes incoming stats, attaches them to `WorkflowRun.mcp_runners`. Called by `_persist_run` on the normal path (before aggregation so the rollup picks them up) and by the safety try/finally in `run()`/`resume()` so exceptions during `_execute_steps` don't leak subprocesses. Idempotent — pool returns `[]` once already drained |
...
| 7.1 `/api/system/extensions` endpoint | ✅ PR #120 | One-stop GET surfaces deployment-resolved plugin paths (system + user, with `user_writable` probe), MCP registry path + binaries dir (from Phase 1.3), live runner-pool state (active count, total RSS overhead, per-runner snapshot from Phase 2), and cache paths; returns 503 when `detect_deployment()` hasn't run yet |
| 6.1 RSS sampling for stdio runners | ✅ PR #117 | `StdioMCPRunner.current_rss_mb()` shipped with the warm-runner pool; `total_runner_overhead_mb()` aggregates across the pool |
| 6.2 Arch scheduler consumes MCP overhead | ✅ PR #121 | Module-level `mcp_overhead_gb()` helper in `api/services/deployment.py` reads the live pool overhead (returns 0.0 if the pool isn't importable or accounting fails — never blocks a memory query); every `Deployment.effective_memory_gb()` impl (DMG / container / host_native) subtracts it after headroom, clamped at 0; the `/api/system/architecture` deployment payload exposes the new `mcp_overhead_gb` field so operators can see why the budget dropped |
| 4.1 `StepResult` instrumentation schema (`SkillActivation` / `MCPCall` / `PluginToolCall` models + `skills_activated` / `mcp_calls` / `plugin_tools_called` / `extension_overhead_ms` fields) | ✅ PR #122 | Schema-only PR; all fields default to empty/0 so pre-Phase-4 runs deserialize unchanged. `MCPCall.status` covers `ok`/`error`/`timeout`; `PluginToolCall` captures `error_class` for triage |
| 4.4 Run-level aggregation | ✅ PR #122 | `aggregate_extension_stats(run)` rolls per-step arrays into `WorkflowRun.{skills_activated_total, mcp_invocations_total, plugin_tools_invoked_total, mcp_servers_used, extension_overhead_seconds}`; idempotent; `mcp_servers_used` dedupes preserving first-seen order. Hooked into `workflow_engine._persist_run` so every terminal persist (success or failure) emits a populated rollup |
| 4.2 plugin tool capture | ✅ PR #127 | `HookContext` gains `step_result` so hooks can append `PluginToolCall` records directly onto the in-flight `StepResult`. `step_executor` passes the result on both `before_step` dispatches. `plugin_tool_invoker` times each `service.call_tool` (`for_each` mode included), records ok/error with `error_class`, and bumps `extension_overhead_ms` so the Phase 4.4 rollup picks them up |
| 2c.1 `lightweight` role in model resolver | ✅ this PR | Adds a sub-3B role pattern (qwen2.5:1.5b, phi, llama3.2:3b, gemma:2b, qwen2.5:0.5b) to `ROLE_PATTERNS` — the substitution target for archetype-aware downsizing on contention steps and the compiler's default for tool-bound steps with no declared model |
| 2c.2 Step archetype registry | ✅ PR #123 | `api/services/archetypes.py` ships 9 built-in archetypes (bash_script, code_review, documentation, research_brief, extraction, synthesis, data_lookup, xsiam_analysis, triage) each with default_role / companion_mcps / companion_skills / trigger_mcps / trigger_skills; `infer_archetype` scores MCP triggers × 2 + skill triggers + role bonus; threshold = 2; `extend_archetypes` lets plugin manifests register custom archetypes but refuses to override built-ins |
| 2c.3 Composer-assist endpoint | ✅ PR #123 | `POST /api/workflows/composer/assist` accepts a partial step dict and returns `{inferred_archetype, suggested_role, suggested_mcps, suggested_skills, warnings}`; subtracts what the operator already declared; warns when declared role doesn't match the inferred archetype's default; tolerates malformed payloads (returns empty shape rather than 422) so the composer keeps working as the user types |
| 2b Co-scheduler / resource-maximization compiler pass | ✅ this PR | `api/services/co_scheduler.py` computes per-step pressure (model footprint × KV headroom + MCP RSS estimate) against the deployment's effective budget × 85%; flags contention steps; picks an action per priority: archetype-aware substitution → split → reorder → block. `apply_co_scheduling_policy` honors the `WorkflowDefaults.co_scheduling_policy` literal (`off` / `recommend` / `warn_strict` / `reject` / `auto_substitute`). `WorkflowEngine.validate` runs the pass and stashes the result on `_last_co_scheduling_result`; `/api/workflows/validate` surfaces `optimization_recommendations` in the success body. `reject` policy promotes to validation errors |
| 7.2 Container security defaults | ✅ this PR | `docker-compose.yml` api service drops all Linux capabilities (`cap_drop: [ALL]`), forbids privilege escalation (`security_opt: [no-new-privileges:true]`), mounts `/tmp` as a 128 MB bounded tmpfs. `read_only: true` posture documented as opt-in for the operator who's confident their workflows don't surprise-write to `/app`. Test suite pins the invariants so a future compose refactor can't silently drop them |
| 7.3 DMG entitlements | ✅ this PR | `desktop/entitlements.plist` explicitly opts out of sandbox (`app-sandbox: false`), grants JIT + unsigned-executable-memory + disable-library-validation (py2app + embedded Python need these) + network client/server. New `docs/deployment/dmg-mcp-security.md` documents what stdio MCP runners inherit, the trust boundary (user-installed plugins ARE code, not data), and the recipe for a sandboxed App Store build (filed against 1.4.x) |
| 4.3 MCP tool capture | ✅ this PR | New `api/hooks/builtins/mcp_tool_invoker.py` mirrors `plugin_tool_invoker` for MCP-protocol tools. Routes through `MCPService.invoke_tool(run_id=…, scope=…)` so calls share the warm pool; records `MCPCall` (ok / error / timeout + `request_size_bytes` / `response_size_bytes` / `error_message`) onto `ctx.step_result.mcp_calls`; `for_each` mode emits one record per iteration. Engine factory + workflow validator both recognize the new hook so YAML workflows can declare it under `hooks.before_step` and reference `<step_id>.<store_as>` as an input |
| 4.2 skill activation capture | ✅ this PR | New `api/hooks/builtins/skill_injector.py` at `transform_prompt` stage. Honors the `mode` literal (`off` / `auto` / `explicit` / `manual`): explicit refs from `step.skills` always inject; auto mode also runs `PluginService.get_skills(prompt.user)` keyword match. Injection site is per-skill (`inject: system` prepends to `prompt.system`; `inject: messages` appends to `prompt.user`); explicit refs take precedence over keyword matches so the same skill never double-injects. New `PluginService.get_skill(plugin_id, skill_id)` helper. `max_skills` cap (default 3) prevents prompt bloat. Each activation records a `SkillActivation` onto `ctx.step_result.skills_activated` with `trigger` (`keyword`/`explicit`), `trigger_match`, `injected_into`, `injected_chars`; `extension_overhead_ms` accumulates the per-step render time |
