...
| 2c.2 Step archetype registry | ✅ this PR | `api/services/archetypes.py` ships 9 built-in archetypes (bash_script, code_review, documentation, research_brief, extraction, synthesis, data_lookup, xsiam_analysis, triage) each with default_role / companion_mcps / companion_skills / trigger_mcps / trigger_skills; `infer_archetype` scores MCP triggers × 2 + skill triggers + role bonus; threshold = 2; `extend_archetypes` lets plugin manifests register custom archetypes but refuses to override built-ins |
| 2c.3 Composer-assist endpoint | ✅ this PR | `POST /api/workflows/composer/assist` accepts a partial step dict and returns `{inferred_archetype, suggested_role, suggested_mcps, suggested_skills, warnings}`; subtracts what the operator already declared; warns when declared role doesn't match the inferred archetype's default; tolerates malformed payloads (returns empty shape rather than 422) so the composer keeps working as the user types |
| 2b Compiler region promotion / co-scheduler | ⬜ not started | Needs the leaf-step pressure walk + archetype-aware action picker; archetypes (2c.2) are now ready as the substitution vocabulary |
