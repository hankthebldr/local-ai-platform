...
| 4.2 skill activation capture | ⬜ follow-up | The engine doesn't yet inject `PluginService.get_skills()` matches into prompts at compose time — there's no integration point to capture from. When skill injection lands (likely a `skill_injector` hook at `transform_prompt` stage), the `HookContext.step_result` channel is ready to receive `SkillActivation` records the same way 4.2/4.3 do |
