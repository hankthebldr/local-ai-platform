...
| 4.2 + 4.3 (engine-side capture in step_executor) — plugin tools | ✅ this PR | `HookContext` gains `step_result` so hooks can append `PluginToolCall` records directly onto the in-flight `StepResult`. `step_executor` passes the result on both `before_step` dispatches. `plugin_tool_invoker` times each `service.call_tool` (`for_each` mode included), records ok/error with `error_class`, and bumps `extension_overhead_ms` so the Phase 4.4 rollup picks them up. Skill activations + MCP-call capture follow the same pattern — they wait on the engine actually injecting skills and invoking MCPs from `step.tools[]` |
...
| 4.2 + 4.3 — MCP / skill capture | ⬜ follow-up | Plugin tool capture is in (above). MCP and skill activation capture wait for the engine to actually invoke MCPs from `step.tools[]` and inject skills into prompts; both will write to existing `StepResult.mcp_calls` / `StepResult.skills_activated` fields via the same `HookContext.step_result` channel |
