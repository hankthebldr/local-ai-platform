    ...
    while True:
        try:
            ...
            if user_input.lower() == "/clear":
                conversation_history = []
                console.print(
                    "[bright_green]✓ Conversation history cleared[/bright_green]"
                )
                continue
            ...
            if user_input.lower() == "/models":
                try:
                    ...
                    for m in models:
                        console.print(
                            f"  [bright_cyan]•[/bright_cyan] [bright_white]{m['name']}[/bright_white]"
                        )
                except Exception as e:
                    console.print(
                        f"[bright_red]✗ Error listing models: {e}[/bright_red]"
                    )
                continue
            ...
            ollama_request = {"model": model, "prompt": prompt, "stream": False}

            response = requests.post(
                f"{host}/api/generate", json=ollama_request, timeout=300
            )
            response.raise_for_status()
            ...
            console.print(
                "[bright_yellow]💡 Make sure Ollama is running:[/bright_yellow] [bright_white]ollama serve[/bright_white]"
            )
            continue
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--model", "-m", default="mistral", help="Model to use (default: mistral)"
    )
    parser.add_argument(
